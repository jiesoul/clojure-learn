(ns com.jiesoul.programingclojure.cryptovault
  (:use [com.jiesoul.programingclojure.protocols :only [IOFactory make-reader make-writer]])
...
(defn vault-key [vault]
  (let [password (.toCharArray (.password vault))]
    (with-open [fis (FileInputStream. (.keystore vault))]
      (-> (doto (KeyStore/getInstance "JECKS")
            (.load fis password))
